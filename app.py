...
USER_PROMPT_TEMPLATE = """위의 양식에 맞춰 업로드된 파일들을 분석하여 EBM 워크시트를 완성해주세요.
...
def encode_file(file_bytes: bytes, file_type: str) -> dict:
    """파일을 Claude API 형식으로 인코딩"""
...
def get_file_media_type(filename: str, content_type: str) -> str:
...
def run_analysis(api_key: str, paper_file, feedback_files: list) -> str:
    ...
    media_type = get_file_media_type(paper_file.name, paper_file.type)
    content.append({"type": "text", "text": "**[새 논문 파일]**"})
    content.append(encode_file(paper_bytes, media_type))

    # 피드백 파일들 추가
    for i, fb_file in enumerate(feedback_files):
        fb_bytes = fb_file.read()
        fb_media_type = get_file_media_type(fb_file.name, fb_file.type)
        content.append({"type": "text", "text": f"**[이전 피드백 파일 {i+1}]**"})
        content.append(encode_file(fb_bytes, fb_media_type))
...
def main():
    ...
    with st.sidebar:
        ...
        # Streamlit Cloud secrets → 환경변수 순으로 시도
        env_key = st.secrets.get("ANTHROPIC_API_KEY", "") if hasattr(st, "secrets") else ""
        if not env_key:
...
""")
    ...
    with col1:
        ...
        paper_file = st.file_uploader(
            ...
            type=["pdf"],
            key="paper",
            help="이번 블록에 배정된 논문 PDF 파일을 업로드하세요.",
        )
        if paper_file:
    ...
    with col2:
        ...
        feedback_files = st.file_uploader(
            "채점 피드백 파일 (PDF 또는 이미지, 여러 개 가능)",
            type=["pdf", "jpg", "jpeg", "png", "webp"],
            accept_multiple_files=True,
            key="feedback",
            help="이전 블록에서 감점된 과제 파일이나 조교 피드백 이미지를 업로드하세요.",
        )
        if feedback_files:
            st.success(f"✅ {len(feedback_files)}개 파일 업로드됨")
    ...
    if st.button(
        ...
        with st.spinner("AI가 논문과 피드백을 분석 중입니다... (1-3분 소요)"):
            try:
                result = run_analysis(api_key, paper_file, feedback_files or [])
                st.session_state["result"] = result
            except anthropic.AuthenticationError:
                st.error("❌ API 키가 올바르지 않습니다. 키를 확인해주세요.")
