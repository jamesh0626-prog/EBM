...
def _get(text, pattern) -> str:
    """**패턴:** 값 추출 — pattern에 | 가 있어도 안전하게 (?:...) 로 감쌈"""
    if not text:
        return ""
    # (?:pattern) 으로 감싸서 | 가 있어도 regex 전체를 분리하지 않음
    m = re.search(
        rf'\*\s*\*\*(?:{pattern})[:\*]*\*\*\s*(.+?)(?=\n\*|\n##|\n###|\Z)',
...
def _iv(text, pattern):
    ...
    pattern 에 | 가 있는 경우 (?:...) 로 감싸서 regex 분리 방지
    """
    m = re.search(
    ...
    if m:
        ans = (m.group(1) or "").replace('[', '').replace(']', '').strip()
        ev = _clean(m.group(2) or "")
        return ans, ev
    # 패턴 매칭 실패 시 더 넓은 패턴 시도
    m2 = re.search(
        rf'(?:{pattern}).*?(\[예\]|\[아니오\]|예|아니오).*?근거[:\s]+(.+?)(?=\n\*|\n##|\Z)',
        text, re.DOTALL | re.IGNORECASE
    )
    if m2:
        ans = (m2.group(1) or "").replace('[', '').replace(']', '').strip()
        ev = _clean(m2.group(2) or "")
        return ans, ev
    return "", ""
...
def generate_docx(ai_text: str) -> bytes:
    ...
    for i, (label, pat) in enumerate(pico_items):
        val = _get(pico_sec, pat) or _get(ai_text, pat) or "(미입력)"
    ...
    # 번호 목록 형태로 된 키워드도 파싱
    if not kw_raw:
        kw_lines = re.findall(r'^\s*[0-9]+[.\)]\s*(.+?)$', kw_sec or ai_text, re.MULTILINE)
        kw_raw = ", ".join(kw_lines)
    keywords = [k.strip() for k in re.split(r'[,;，\n]', kw_raw) if k.strip()][:5]
    for i in range(5):
        kw = keywords[i] if i < len(keywords) else ""
        _para(doc, f"{i+1}.  {kw}", sa=5)
    ...
    iv_qs = [
        ...
        ("환자는 모두 애초에 무작위 배정된 군에 따라 분석되었는가?\n(Intention to treat analysis)",
         r"(?:Intention to treat|ITT)"),   # ← | 를 (?:...) 로 감쌈
        ("이중 맹검법이 시행되는가?",
         r"이중 맹검"),
    ...
    for i, (q_label, q_pat) in enumerate(iv_qs):
        ...
        if ev:
            lines.append((f"[근거] {ev[:150]}", False, (80, 80, 80)))
        else:
            lines.append(("(근거 없음)", False, (150, 150, 150)))
    ...
    out_label = _get(imp_text, r"Outcome\s*항목") or _get(ai_text, r"Outcome\s*항목") or "(Primary Outcome 미기재)"
    cer_val   = _get(imp_text, r"CER") or _get(ai_text, r"CER") or "(미기재)"
    eer_val   = _get(imp_text, r"EER") or _get(ai_text, r"EER") or "(미기재)"
    rrr_val   = _get(imp_text, r"RRR") or _get(ai_text, r"RRR") or "(미기재)"
    arr_val   = _get(imp_text, r"ARR") or _get(ai_text, r"ARR") or "(미기재)"
    nnt_val   = _get(imp_text, r"NNT\s*\(Number") or _get(ai_text, r"NNT\s*\(Number") or "(미기재)"

    # 통계 표: Outcome | CER | EER | RRR | ARR | NNT (header + values)
    st_tbl = doc.add_table(rows=2, cols=6)
    _borders(st_tbl)
    ...
    for j, (h, v, w) in enumerate(zip(headers, values, widths)):
        ...
        _set_row_height(st_tbl.rows[1], 1.2)
    ...
    ci_val = _clean(ci_m.group(1)) if ci_m else "(미기재)"
    se_m = re.search(r'S\.?E\.?\s*=\s*(.+?)(?=\n|\Z)', ai_text, re.DOTALL | re.IGNORECASE)
    se_val = _clean(se_m.group(1)) if se_m else "(미기재)"
    ...
    adv_text = adv_sec.group(1) if adv_sec else ""

    adv_item = _get(adv_text, r"주요 부작용 항목") or _get(ai_text, r"주요 부작용 항목") or "(미기재)"
    # 부작용 섹션에서 CER/EER 별도 추출
    adv_cer  = _get(adv_text, r"CER") or "(미기재)"
    adv_eer  = _get(adv_text, r"EER") or "(미기재)"
    rri_val  = _get(adv_text, r"RRI") or _get(ai_text, r"RRI") or "(미기재)"
    ari_val  = _get(adv_text, r"ARI") or _get(ai_text, r"ARI") or "(미기재)"
    nnh_val  = _get(adv_text, r"NNH\s*\(Number") or _get(ai_text, r"NNH\s*\(Number") or "(미기재)"

    adv_tbl = doc.add_table(rows=2, cols=6)
    _borders(adv_tbl)
    ...
    for j, (h, v) in enumerate(zip(adv_hdrs, adv_vals)):
        ...
        _set_row_height(adv_tbl.rows[1], 1.2)
    ...
    # 예/아니오 두 질문
    q1_m = re.search(
        r'실제환자와 연구대상.+?(\[예\]|\[아니오\]|예|아니오).+?이유[:\s]+(.+?)(?=\n\*|\n##|\Z)',
        ai_text, re.DOTALL
    )
    q2_m = re.search(
        r'다른 치료법은 없는가.+?(\[예\]|\[아니오\]|예|아니오).+?이유[:\s]+(.+?)(?=\n\*|\n##|\Z)',
        ai_text, re.DOTALL
    )
    q1_ans = (q1_m.group(1) or "").replace('[', '').replace(']', '') if q1_m else "(미기재)"
    q1_ev  = _clean(q1_m.group(2) or "") if q1_m else "(이유 미기재)"
    q2_ans = (q2_m.group(1) or "").replace('[', '').replace(']', '') if q2_m else "(미기재)"
    q2_ev  = _clean(q2_m.group(2) or "") if q2_m else "(이유 미기재)"
    ...
    f_t   = ft_m.group(1) if ft_m else "(미기재)"
    f_a   = fa_m.group(1) if fa_m else "(미기재)"
    nnt_pt = _clean(nnt_p.group(1)) if nnt_p else "(미기재)"
    nnh_pt = _clean(nnh_p.group(1)) if nnh_p else "(미기재)"

    # f-factor 설명 (행 2에 사용)
    ft_reason = _get(app_text, r"f.*?treatment") or _get(ai_text, r"f.*?treatment") or ""
    fa_reason = _get(app_text, r"f.*?adverse") or _get(ai_text, r"f.*?adverse") or ""

    # s factor, LHH, rating scale 1)/2) 값
    s_m    = re.search(r'\bs\s*(?:factor\s*)?[=:]\s*([0-9.]+)', ai_text, re.IGNORECASE)
    lhh_m  = re.search(r'LHH\s*=\s*(.+?)(?=\n##|\n###|\Z)', ai_text, re.DOTALL | re.IGNORECASE)
    rs1_m  = re.search(r'Rating\s*scale\s*1\).*?[=:]\s*([0-9.]+)', ai_text, re.IGNORECASE)
    rs2_m  = re.search(r'Rating\s*scale\s*2\).*?[=:]\s*([0-9.]+)', ai_text, re.IGNORECASE)
    s_val  = s_m.group(1) if s_m else ""
    lhh_val = _clean(lhh_m.group(1)) if lhh_m else "(미기재)"
    s1_val = rs1_m.group(1) if rs1_m else ""
    s2_val = rs2_m.group(1) if rs2_m else ""

    # s factor 설명 (행 5에 사용)
    s_reason = _get(app_text, r"s.*?factor") or _get(ai_text, r"s.*?factor") or ""

    # 결론
    conc_m = re.search(r'결론[:\s]*(.+?)(?=\n##|\n###|\Z)', ai_text, re.DOTALL)
    conc_val = _clean(conc_m.group(1)) if conc_m else "(미기재)"

    # 적용 가능성 표 (7행)
    app_tbl = doc.add_table(rows=7, cols=2)
    _borders(app_tbl)
    ...
    _app_row(0,
        ...
        f"{q1_ans}\n\n이유: {q1_ev[:150]}" if q1_ev else q1_ans)
    _app_row(1,
        "다른 치료법은 없는가?",
        f"{q2_ans}\n\n이유: {q2_ev[:150]}" if q2_ev else q2_ans)
    _app_row(2,
        "해당 치료법 적용시 실제 환자에서 기대되는 잠재적인 편익과 손실은 무엇인가?\n(NNT와 NNH를 구하여 평가한다.)",
        _clean(ft_reason[:200]) if ft_reason else "(f-factor 평가 아래 참조)")
    _app_row(3,
        "① 'f'를 정한다.",
    ...
    _app_row(5,
        ...
        _clean(s_reason[:200]) if s_reason else "(s-factor 평가 아래 참조)")
    _app_row(6,
        "① 's' factor를 구한다. (주어진 scale을 이용할 것)\n② 앞서 구한 f와 s를 이용하여 LHH를 구한다.",
        f"s factor = {s_val if s_val else '(미기재)'}\nLHH = [(1/NNT)×f×s] vs. [(1/NNH)×f]\n{lhh_val[:200]}")
    for r in range(7):
    ...
    _borders(scale_tbl1)
    # 1)값 위치를 셀 3(0.5~1.0 구간)에 표시
    cell3_label1 = f"▲ {s1_val}" if s1_val else ""
    for j, (val, lbl) in enumerate(zip(
            ["0", "0.1", "0.5", cell3_label1, "1.0"],
            ["Outcome", "", "", "", "완치(건강)"])):
    ...
    if s1_val:
        _para(doc, f"  1) 값: {s1_val}", size=10, sb=2, sa=2)
    ...
    _borders(scale_tbl2)
    cell3_label2 = f"▲ {s2_val}" if s2_val else ""
    for j, (val, lbl) in enumerate(zip(
            ["0", "0.1", "0.5", cell3_label2, "1.0"],
            ["수용 못함", "", "", "", "수용 함"])):
    ...
    if s2_val:
        _para(doc, f"  2) 값: {s2_val}", size=10, sb=2, sa=2)

    doc.add_paragraph()

    # s factor 분수 표시
    s_display = s_val if s_val else "?"
    s2_display = s2_val if s2_val else "2)값"
    s1_display = s1_val if s1_val else "1)값"
    _para(doc, f"{s2_display}          s = {s_display}", size=11, sb=6, sa=0)
    _para(doc, "──────────────────", size=11, sb=0, sa=0)
    _para(doc, s1_display, size=11, sb=0, sa=10)

    # 결론
    p_conc = doc.add_paragraph()
    p_conc.paragraph_format.space_before = Pt(10)
    r1 = p_conc.add_run(
        "결론 :  이제까지 시행한 분석에 근거하여 당신은 환자에게 위의 치료를 하도록 권고할 것인가?   "
    )
    _font(r1, size=10.5)
    conc_yn = "예" if "예" in conc_val[:10] else "아니오" if "아니오" in conc_val[:15] else ""
    r2 = p_conc.add_run("예  /  아니오")
    ...
    # disc_full에서 직접 번호 없이 파싱 시도 (패턴 매칭 실패 시)
    if not disc1 and disc_full:
        parts = re.split(r'\n\s*[12][.\)]\s*\*\*', disc_full)
        if len(parts) >= 2:
            disc1 = _clean(parts[1].split('**')[0] + parts[1].split('**', 1)[-1] if '**' in parts[1] else parts[1])
        if len(parts) >= 3:
            disc2 = _clean(parts[2].split('**')[0] + parts[2].split('**', 1)[-1] if '**' in parts[2] else parts[2])
    if not disc1:
        disc1 = "(Discussion 1 - AI 응답에서 추출 실패. 원문 보기 탭을 참조하세요.)"
    if not disc2:
        disc2 = "(Discussion 2 - AI 응답에서 추출 실패. 원문 보기 탭을 참조하세요.)"

    _para(doc, "1. 앞에서 분석한 논문이 가지고 있는 문제점이나 제한점에는 어떠한 것들이 있겠는가?",
          bold=False, size=10.5, sb=4, sa=6)
    _para(doc, disc1, size=10.5, indent=0.5, sa=20)

    _para(doc, "2. 이러한 제한점을 보완하기 위하여 어떤 근거가 더 필요하겠는가?",
          bold=False, size=10.5, sb=4, sa=6)
    _para(doc, disc2, size=10.5, indent=0.5, sa=20)
